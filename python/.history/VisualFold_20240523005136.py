...
def PlotOri(Node, Panel, Trigl, linestyle='-', linewidth=0.3, color='none'):
    """
    Plot the original structure.
    """
    for i in range(Trigl.shape[0]):
        plt.plot([Node[Trigl[i, 0], 0], Node[Trigl[i, 1], 0], Node[Trigl[i, 2], 0]],
                [Node[Trigl[i, 0], 1], Node[Trigl[i, 1], 1], Node[Trigl[i, 2], 1]],
                [Node[Trigl[i, 0], 2], Node[Trigl[i, 1], 2], Node[Trigl[i, 2], 2]],
