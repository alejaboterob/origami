...
file_path = r"C:\Users\mboter45\OneDrive - Universidad EAFIT\Estructuras origami\code\origami\python\fold\flat_crane _ 90PercentFolded.fold"
Nodes, Panels, BDRY = readFOLD(file_path)
...
plot_panels(Nodes, Panels, ax, plot_nodes=True, **plot_kwargs)
...
side = np.array([22,23,34,36, 51,42,44,47])
...
# indp = Load[:, 0]
side2 = np.array([3,8])
Load = np.vstack((side2.T, np.zeros_like(side2).T, -np.ones_like(side2).T*0.1, np.zeros_like(side2).T)).T
